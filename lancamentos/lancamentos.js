document.addEventListener('DOMContentLoaded', () => {
  ...
  const dropdownMenu = document.querySelector(
    '#dateFilterDropdown + .dropdown-menu'
  )
  ...
  const entradaModal = new bootstrap.Modal(
    document.getElementById('entradaModal')
  )
  ...
  function carregarContasNosSelects() {
    ...
    const entradaConta = document.getElementById('entradaConta')
    const saidaConta = document.getElementById('conta')

    entradaConta.innerHTML = '<option value="">Selecione uma conta</option>'
    saidaConta.innerHTML = '<option value="">Selecione uma conta</option>'

    contasSalvas.forEach(conta => {
      const optionEntrada = document.createElement('option')
      optionEntrada.value = conta.nome
      optionEntrada.textContent = conta.nome
      entradaConta.appendChild(optionEntrada)

      const optionSaida = document.createElement('option')
      optionSaida.value = conta.nome
      optionSaida.textContent = conta.nome
      saidaConta.appendChild(optionSaida)
    })
  }
  ...
  const updateTable = () => {
    ...
    const filteredLancamentos = lancamentos.filter(item => {
      ...
      const filteredLancamentos = lancamentos.filter(item => {
        if (currentFilter === 'todos') return true
        if (currentFilter === 'contas')
          return item.conta === 'Conta 1' || item.conta === 'Conta 2'
        if (currentFilter === 'investimentos')
          return item.conta === 'Investimentos'
        if (currentFilter === 'cartao')
          return item.conta === 'Cartão de Crédito'
        return false
      })
    })
    ...
    filteredLancamentos.forEach(item => {
      ...
      row.innerHTML = `
                ...
                <td>${item.conta}</td>
                <td style="color: ${
                  item.tipo === 'entrada' ? 'var(--green)' : 'var(--red)'
                };">${formattedValor}</td>
                <td></td>
            `
  ...
  dropdownMenu.addEventListener('click', e => {
    ...
    if (target.classList.contains('dropdown-item')) {
      ...
      if (
        ...
        ['Últimos 30 dias', 'Últimos 60 dias', 'Últimos 90 dias'].includes(
          selectedValue
        )
      ) {
        filterType = 'period'
